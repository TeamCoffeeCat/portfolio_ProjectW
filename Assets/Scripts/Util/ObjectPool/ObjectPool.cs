...
namespace ProjectW.Util
{
    ...
    public class ObjectPool<T> where T : MonoBehaviour, IPoolableObject
    {
        ...
        /// <summary>
        /// 풀에서 재사용 가능한 객체가 존재하는지를 나타내는 프로퍼티
        ///  -> Pool.Find(_ => _.CanRecycle)
        ///     풀 안에 있는 객체 중 CanRecycle 값이 true인 객체가 있는지 찾는 작업
        ///     풀 안에 있는 T타입 객체는 IPoolableObject를 상속받으므로 CanRecycle 프로퍼티를 가짐
        /// </summary>
        ...
        /// </summary>
        /// <param name="obj">반환하고자 하는 T타입 인스턴스</param>
        ...
        /// 풀 내의 재사용 가능한 객체를 반환
        /// </summary>
        /// <returns>재사용 가능한 T타입 인스턴스</returns>
        public T GetObj()
        {
            // 해당 메서드는 특정한 조건의 객체를 찾는 것이 아니고
            // 재사용 가능하다면 해당 타입의 객체를 반환받아 사용하는 용도
            return GetObj(_ => _.CanRecycle);
        }
        ...
        public T GetObj(Func<T, bool> condition)
        {
            // 풀 내에서 재사용 가능한 객체가 없다면
            if (!canRecycle)
            {
                // 풀에서 조건과 동일한 객체가 있을 경우 반환, 없다면 null
                var protoObj = Pool.Find(_ => condition(_));

                // 재사용이 불가능하지만, 조건을 만족하는 객체가 있다면
                if (protoObj != null)
                {
            ...
            // 재사용 가능하고, 파라미터로 받은 조건에 해당하는 객체를 찾음
            T recycleObj = Pool.Find(_ => condition(_) && _.CanRecycle);
