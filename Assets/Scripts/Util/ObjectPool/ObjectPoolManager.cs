    ...
    public class ObjectPoolManager : Singleton<ObjectPoolManager>
    {
        ...
        public void RegistPool<T>(T obj, int poolCount = 1) where T : MonoBehaviour, IPoolableObject
        {
            ...
            if (poolDic.ContainsKey(key))
            {
                ...
                //         기존에 있던 동일한 몬스터풀을 이용할 수 있게 하려고 함.
                pool = poolDic[key] as ObjectPool<T>;
            }
            // 존재하지 않는다면
            else
            {
                // 해당 키 값의 풀이 등록되어 있지 않으므로, 새로 생성하여 딕셔너리에 추가
            ...
            if (pool.holder == null)
            {
                ...
                //     이 말은 상황에 따라 적절히 풀이 필요 없어질 때 본인이 해제해야함.
        ...
        {
            // 이 기능은 특정 씬에서 들고 있을 필요가 없는 풀이 발생했을 떄
            // 해당 풀을 비우기 위한 용도로 사용
